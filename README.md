...
这是一个基于分数表示法的命令行交互式线性代数计算辅助程序，采用C++17开发，使用了部分boost库的特性。系统使用任意精度分数运算确保计算的精确性，提供了简单的线性代数功能和友好的终端用户界面。
vibe coding这个项目主要是因为本人算矩阵题时往往不知道自己计算结果是否正确，MMA感觉太重了，用来做矩阵计算有点大炮打蚊子的感觉，输入矩阵命令也比较麻烦，所以就想手搓一个简单的轮子耍耍，不过终究还是低估了代数解析的难度，再加上AI越来越强大，后来发现用多模态AI将矩阵计算题图像识别出来，然后生成py脚本让它自己执行并输出结果更加简单准确，这个项目也就没有任何继续存在的意义了。

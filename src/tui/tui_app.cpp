    ...
    if (key == KEY_ESCAPE) {
        ...
        if (Terminal::hasInput()) {
            ...
            if (next == '[') {
                ...
                switch (code) {
                    case 'A': handleSpecialKey(KEY_UP); break;    // 上箭头
                    case 'B': handleSpecialKey(KEY_DOWN); break;  // 下箭头
                    case 'C': handleSpecialKey(KEY_RIGHT); break; // 右箭头
                    case 'D': handleSpecialKey(KEY_LEFT); break;  // 左箭头
                }
            }
        } else {
            // ESC键

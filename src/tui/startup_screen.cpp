      ...
      bannerFileConfigPath_(bannerFilePath), workDirConfigPath_(workDirPath)
{
    ...
    { // 确保 catch 使用 std::filesystem::filesystem_error 或 fs::filesystem_error
...
void StartupScreen::draw()
{
    ...
    if (termRows != lastTermRows_ || termCols != lastTermCols_)
    {
        ...
        TuiUtils::fillRect(0, 0, termRows, termCols, ' ', Color::DEFAULT, Color::DEFAULT); // 此行保持注释
...
void StartupScreen::drawWideLayout(int termRows, int termCols)
{
    ...
    if (dirWidth < 20)
        dirWidth = std::min(termCols / 2, 20);    // 最小目录宽度
    ...
    if (listH > 0 && listW > 0)
    {
        ...
        TuiUtils::drawTextList(listR, listC, listH, listW, fileList_, currentSelection_, scrollOffset_,
                               ...
                               Color::DEFAULT,             // selectedColor (FG)
                               Color::CYAN,                // selectedBgColor
                               Color::DEFAULT,             // defaultBgColor
                               NULL_WORKSPACE_OPTION_TEXT, // specialItemText
                               Color::YELLOW,              // specialItemFgColor
                               Color::DEFAULT              // specialItemBgColor (与 defaultBgColor 相同)
        );
    }
...
void StartupScreen::drawTallLayout(int termRows, int termCols)
{
    ...
    if (dirHeight < 5)
        dirHeight = std::min(termRows, 5); // 最小目录高度

...
## 目录
1. [简介](#简介)
2. [分数类 (Fraction)](#分数类-fraction)
3. [向量类 (Vector)](#向量类-vector)
4. [矩阵类 (Matrix)](#矩阵类-matrix)
5. [矩阵操作类 (MatrixOperations)](#矩阵操作类-matrixoperations)
6. [操作步骤与历史记录 (OperationStep & OperationHistory)](#操作步骤与历史记录)
7. [使用示例](#使用示例)

## 简介

本线性代数计算系统是一个基于C++的库，用于执行各种线性代数计算，特别注重计算精度和步骤展示。系统使用分数表示法，避免浮点数计算中的精度损失。本文档介绍系统中的核心类和API。
...
这个API文档已更新，包含了线性代数计算系统所有已实现的功能，包括新增的代数余子式矩阵、伴随矩阵和行列式展开计算等功能。用户可以参考此文档快速了解和使用系统的各项功能。
